...
const globalForOrderNonce = globalThis as unknown as {
  orderNonceStore?: Map<string, NonceRecord>;
};

const store =
  globalForOrderNonce.orderNonceStore ?? new Map<string, NonceRecord>();

globalForOrderNonce.orderNonceStore = store;

function normalizeAddress(address: string) {
  return address.toLowerCase() as `0x${string}`;
}

export function createOrderNonce(address: `0x${string}`) {
...
export function consumeOrderNonce(params: {
  address: `0x${string}`;
  nonce: `0x${string}`;
}) {
